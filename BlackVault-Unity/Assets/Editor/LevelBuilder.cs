...
public static class LevelBuilder
{
    ...
    public static void BuildLevel1()
    {
        ...
        GameObject terminal = BuildTerminal(door);
        GameObject player = BuildPlayer();
        GameObject canvas = BuildMLPuzzleCanvas(player);

        WireTerminal(terminal, canvas, door);

        Debug.Log("[BlackVault] Level 1 scene built. Save it as " +
                  "01_Level1_DataCleaning.unity, then check the " +
    ...
    // Terminal
    // ------------------------------------------------------------------
    private static GameObject BuildTerminal(GameObject door)
    {
        ...
        interactable.datasetId = "house_prices"; // ADJUST to match your Level 1 mission config
